      ...
      <a-card :title="item.groupKey ? '分组' : '未分组字段' " w-973px>
        ...
        <div v-if="item.groupKey" >
          ...
          <div>
            ...
            <a-button type="dashed" block @click="addField(cardIndex)" mt>
              <PlusOutlined />
              添加组内字段
            </a-button>
          </div>
...
const removeCard = (item: ModelInfo) => {
  const index = formState.modelConfig!.models!.indexOf(item);
  if (index !== -1) {
    formState.modelConfig!.models!.splice(index, 1);
  }
};
