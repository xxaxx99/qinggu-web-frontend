<template>
        ...
        <a-card :title="item.groupKey ? '分组' : '未分组字段' " w-833px>
          ...
          <div v-if="item.groupKey"  >
            ...
            <div>
              ...
              <a-button type="dashed" block @click="addField1(cardIndex)">
                <PlusOutlined />
                添加组内字段
              </a-button>
            </div>
  ...
  <template v-else>
    <a-form
        ref="formRef"
        name="dynamic_form_nest_item"
        :model="oldData.fileConfig!.files"
    >
      <a-space
          v-for="(item , cardIndex) in oldData.fileConfig!.files"
          :key="item.groupKey"
          style="display: flex; margin-bottom: 8px"
          align="baseline"
          justify-center
        ...
        <a-card :title="item.groupKey ? '分组' : '未分组字段' " w-833px>
          ...
          <div v-if="item.groupKey"  >
            ...
            <div>
              ...
              <a-button type="dashed" block @click="addField2(cardIndex)">
                <PlusOutlined />
                添加组内字段
              </a-button>
            </div>
...
const addField2 = (cardIndex: number) => {
  if(typeof formState.fileConfig!.files![cardIndex].files == 'undefined'){
    oldData.value.fileConfig!.files![cardIndex].files = [{}]
  }else {
    oldData.value.fileConfig!.files![cardIndex].files?.push({})
  }
}
// 组内删除字段
...
const removeField2 = (cardIndex: number, fieldIndex: number) => {
  oldData.value.fileConfig!.files![cardIndex].files?.splice(fieldIndex, 1);
}
...
const addCard = (addType: "field" | "group") => {
  switch (addType) {
    case "field":
      ...
      oldData.value.fileConfig!.files!.push({})
      break
    case "group":
      formState.fileConfig!.files!.push({ groupKey: "groupKey" , groupName: "分组"})
      oldData.value.fileConfig!.files!.push({ groupKey: "groupKey" , groupName: "分组"})
      break
  }
}
...
const removeCard = (item: ModelInfo) => {
  const index1 = formState.fileConfig!.files!.indexOf(item);
  const index2 = oldData.value.fileConfig!.files!.indexOf(item);
  if (index1 !== -1) {
    formState.fileConfig!.files!.splice(index1, 1);
  }
  if (index2 !== -1) {
    formState.fileConfig!.files!.splice(index2, 1);
  }
};
