      ...
      <div class="ant-pro-form-login-container">
        ...
        <div class="ant-pro-form-login-main" w-335px>
          <a-form ref="formRef" :model="registerModel">
            ...
            <!-- 判断是否存在error -->
            <a-alert v-if="errorAlert && registerModel.type === 'account'" mb-24px :message="t('pages.login.accountLogin.errorMessage')" type="error" show-icon />
            <a-alert v-if="errorAlert && registerModel.type === 'mobile'" mb-24px :message="t('pages.login.phoneLogin.errorMessage')" type="error" show-icon />
            <template v-if="registerModel.type === 'account'">
              <a-form-item name="username" :rules="[{ required: true, message: t('pages.login.username.required') }]">
                <a-input v-model:value="registerModel.username" allow-clear :placeholder="t('pages.login.username.placeholder')" size="large" @press-enter="submit">
                  <template #prefix>
                    <UserOutlined />
                  </template>
                </a-input>
              ...
              <a-form-item name="password" :rules="[{ required: true, message: t('pages.login.password.required') }]">
                <a-input-password v-model:value="registerModel.password" allow-clear :placeholder="t('pages.login.password.placeholder')" size="large" @press-enter="submit">
                  <template #prefix>
                    <LockOutlined />
                  </template>
                </a-input-password>
            ...
            <template v-if="registerModel.type === 'mobile'">
              ...
              >
                <a-input v-model:value="registerModel.mobile" allow-clear :placeholder="t('pages.login.phoneNumber.placeholder')" size="large" @press-enter="submit">
                  <template #prefix>
                    <MobileOutlined />
                  </template>
                </a-input>
              ...
              <a-form-item name="code" :rules="[{ required: true, message: t('pages.login.captcha.required') }]">
                <div flex items-center>
                  <a-input
                    v-model:value="registerModel.code" style="flex: 1 1 0%; transition: width 0.3s ease 0s; margin-right: 8px;"
